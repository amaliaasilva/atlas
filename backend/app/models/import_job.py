...
class ImportJob(Base):
    ...
    import_type: Mapped[str] = mapped_column(
        SAEnum(ImportType), nullable=False, default=ImportType.unit_budget
    )
    status: Mapped[str] = mapped_column(
        SAEnum(ImportStatus), nullable=False, default=ImportStatus.pending
    )
    unit_id: Mapped[str | None] = mapped_column(
        String(36), ForeignKey("units.id", ondelete="SET NULL"), nullable=True
    )
    budget_version_id: Mapped[str | None] = mapped_column(
        String(36), ForeignKey("budget_versions.id", ondelete="SET NULL"), nullable=True
    )
    ...
    mappings: Mapped[list["ImportMapping"]] = relationship(
        "ImportMapping", back_populates="import_job", cascade="all, delete-orphan"
    )
...
class ImportMapping(Base):
    ...
    import_job: Mapped["ImportJob"] = relationship(
        "ImportJob", back_populates="mappings"
    )
