...
class LineItemDefinition(Base, TimestampMixin):
    ...
    calculation_type: Mapped[str] = mapped_column(SAEnum(CalculationType), nullable=False, default=CalculationType.formula)
    formula_reference: Mapped[str | None] = mapped_column(Text, nullable=True)  # JSON or string referencing assumption codes
    ...
    # Relationships
    business: Mapped["Business"] = relationship("Business", back_populates="line_item_definitions")
    calculated_results: Mapped[list["CalculatedResult"]] = relationship("CalculatedResult", back_populates="line_item")
