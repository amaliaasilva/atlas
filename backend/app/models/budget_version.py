...
class BudgetVersion(Base, TimestampMixin):
    ...
    status: Mapped[str] = mapped_column(
        SAEnum(VersionStatus), nullable=False, default=VersionStatus.draft
    )
    ...
    scenario: Mapped["Scenario"] = relationship(
        "Scenario", back_populates="budget_versions"
    )
    creator: Mapped["User | None"] = relationship("User", foreign_keys=[created_by])
    assumption_values: Mapped[list["AssumptionValue"]] = relationship(
        "AssumptionValue", back_populates="budget_version", cascade="all, delete-orphan"
    )
